...
namespace OnlineServices.Conn
{
  public class DBConn
  {
    ...
    //public string lngGenCod(string strTable, string strPrimaryKey, clsConn oConn)
    //{
    //  long lngCodigo;
    //  DataTable dtCodigo;
    //  StringBuilder strQuery = new StringBuilder("SELECT 1 FROM " + strTable + " WHERE " + strPrimaryKey + " = ");
    //  do
    //  {
    //    lngCodigo = long.Parse(DateTime.Now.ToString("yyyyMMddHHmmss").ToString());
    //    dtCodigo = oConn.Query(strQuery.ToString() + lngCodigo);
    //  } while (dtCodigo.Rows.Count > 0);
    //  return lngCodigo.ToString();
    //}

    //public string lngGenCodMax(string strTable, string strPrimaryKey, string strWhere, clsConn oConn)
    //{
    //  long lngCodigo;
    //  DataTable dtCodigo;

    //  StringBuilder strQuery = new StringBuilder("select max(" + strPrimaryKey + ") from " + strTable);
    //  if (!string.IsNullOrEmpty(strWhere))
    //  {
    //    strQuery.Append(" where " + strWhere);
    //  }
    //  dtCodigo = oConn.Query(strQuery.ToString());
    //  if (!string.IsNullOrEmpty(dtCodigo.Rows[0][0].ToString()))
    //  {
    //    lngCodigo = long.Parse(dtCodigo.Rows[0][0].ToString());
    //    lngCodigo++;
    //  }
    //  else
    //  {
    //    lngCodigo = 1; // long.Parse(lngGenCod(strTable, strPrimaryKey, oConn));
    //  }
    //  return lngCodigo.ToString();
    //}
