...
{
  public partial class app_cli_retenidos_bus : System.Web.UI.Page
  {
    ...
    protected void rdGridClientesRetenidos_NeedDataSource(object source, GridNeedDataSourceEventArgs e)
    {
      rdGridClientesRetenidos.DataSource = getDatatble();
    }

    protected void rdGridClientesRetenidos_ItemCommand(object source, GridCommandEventArgs e)
    {
      if (e.CommandName == RadGrid.ExportToExcelCommandName)
      {
        ...
        wb.Worksheets.Add(dt, "clientesretenidos");

        Response.Clear();
        Response.Buffer = true;
        Response.Charset = "";
        Response.ContentType = "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet";
        Response.AddHeader("content-disposition", "attachment;filename=clientesretenidos_" + DateTime.Now.ToString("yyyyMMdd") + ".xlsx");
    ...
    public DataTable getDatatble()
    {
      ...
      if (oConn.Open())
      {
        cClientesRetenidos oClientesRetenidos = new cClientesRetenidos(ref oConn);
        oClientesRetenidos.CodNkey = ((!string.IsNullOrEmpty(cmbCliente.SelectedValue) ? cmbCliente.SelectedValue : hdd_arrNkeyCliente.Value));
        oClientesRetenidos.NcodHolding = cmbHolding.SelectedValue;
        oClientesRetenidos.NkeyUsuario = oIsUsuario.NKeyUsuario;
        oClientesRetenidos.TipoUsuario = oIsUsuario.TipoUsuario;
        dt = oClientesRetenidos.Get();
    ...
    protected void idBuscar_Click(object sender, EventArgs e)
    {
      ...
      rdGridClientesRetenidos.Rebind();
    }

    protected void rdGridClientesRetenidos_ItemDataBound(object sender, GridItemEventArgs e)
    {
      if (e.Item is GridDataItem)
      {
        ...
        if (!string.IsNullOrEmpty(Decimales))
        {
          ...
          {
            if ((!string.IsNullOrEmpty(item["deudatotal"].Text)) && (item["deudatotal"].Text != "&nbsp;"))
              item["deudatotal"].Text = double.Parse(row["deudatotal"].ToString()).ToString("N" + Decimales);

            if ((!string.IsNullOrEmpty(item["deudavencida"].Text)) && (item["deudavencida"].Text != "&nbsp;"))
              item["deudavencida"].Text = double.Parse(row["deudavencida"].ToString()).ToString("N" + Decimales);
          }
          else
          {
            if ((!string.IsNullOrEmpty(item["deudatotal"].Text)) && (item["deudatotal"].Text != "&nbsp;"))
              item["deudatotal"].Text = double.Parse(row["deudatotal"].ToString()).ToString("N0");

            if ((!string.IsNullOrEmpty(item["deudavencida"].Text)) && (item["deudavencida"].Text != "&nbsp;"))
              item["deudavencida"].Text = double.Parse(row["deudavencida"].ToString()).ToString("N0");
          }
        }
        else
        {
