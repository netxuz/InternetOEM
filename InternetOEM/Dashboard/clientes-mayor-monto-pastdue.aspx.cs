...
{
  public partial class clientes_mayor_monto_pastdue : System.Web.UI.Page
  {
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
      ...
      get20CliMayMntPastDue();

      Log oLog = new Log();
      oLog.IdUsuario = oIsUsuario.CodUsuario;
      oLog.ObsLog = "TOP 20 CLIENTES CON MAYOR % MONTO PAST DUE";
      oLog.CodEvtLog = "102";
      oLog.AppLog = "TOP 20 CLIENTES CON MAYOR % MONTO PAST DUE";
      oLog.putLog();
    }
    ...
    protected void get20CliMayMntPastDue() {
      ...
      if (oConn.Open())
      {
        ...
        DataTable dt = oTopCliMayMntPastDue.getTop20cli_may_pastdue();
        if (dt != null)
        {
          if (dt.Rows.Count > 0)
          {
            ...
            foreach (DataRow oRow in dt.Rows)
            {
              ...
              sHtml = sHtml + "<div class='blq-dt-pastdue'><span class='dt-pastdue'>$ " + Math.Truncate(double.Parse(oRow["past_due"].ToString())).ToString("N0") + "</span></div>";
              sHtml = sHtml + "</div>";
              sHtml = sHtml + "</a>";
              sHtml = sHtml + "</div>";
