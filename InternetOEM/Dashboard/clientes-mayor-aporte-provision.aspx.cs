...
{
  public partial class clientes_mayor_aporte_provision : System.Web.UI.Page
  {
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
      ...
      get20CliMayMntAporteProvision();

      Log oLog = new Log();
      oLog.IdUsuario = oIsUsuario.CodUsuario;
      oLog.ObsLog = "TOP 20 CLIENTES CON MAYOR APORTE EN PROVISIÓN";
      oLog.CodEvtLog = "104";
      oLog.AppLog = "TOP 20 CLIENTES CON MAYOR APORTE EN PROVISIÓN";
      oLog.putLog();
    }
    ...
    protected void get20CliMayMntAporteProvision()
    {
      ...
      if (oConn.Open())
      {
        ...
        DataTable dt = oTopCliMayMntPastDue.getTopCliMayProvision();
        if (dt != null)
        {
          if (dt.Rows.Count > 0)
          {
            ...
            foreach (DataRow oRow in dt.Rows)
            {
              ...
              sHtml = sHtml + "<div class='blq-dt-pastdue'><span class='dt-pastdue'>$ " + Math.Truncate(double.Parse(oRow["impacto_provision"].ToString())).ToString("N0") + "</span></div>";
              sHtml = sHtml + "</div>";
              sHtml = sHtml + "</a>";
              sHtml = sHtml + "</div>";
