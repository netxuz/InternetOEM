  ...
  public class cSobreGirosLineaCredito
  {
    ...
    public DataTable Get()
    {
      ...
      if (oConn.bIsOpen)
      {
        ...
        cSQL.Append("Select ncodigodeudor, snombre,  fecha, isnull(lineactual,0) as lineactual, isnull(saldo,0) as saldo From saldo_lineacreditoweb Where nkey_cliente = ").Append(lngCodNkey);
        ...
        if ((!string.IsNullOrEmpty(sTipoUsuario)) && (sTipoUsuario == "V"))
          cSQL.Append("  and saldo_lineacreditoweb.nkey_deudor in (select codigodeudor.nkey_deudor from codigodeudor where codigodeudor.nkey_vendedor = ").Append(lngNkeyUsuario).Append(" and codigodeudor.nkey_cliente = ").Append(lngCodNkey).Append(" )  ");
