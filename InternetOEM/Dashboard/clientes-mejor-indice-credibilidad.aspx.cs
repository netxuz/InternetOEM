...
{
  public partial class clientes_mejor_indice_credibilidad : System.Web.UI.Page
  {
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
      ...
      get20CliMejorIndiceCredibilidad();

      Log oLog = new Log();
      oLog.IdUsuario = oIsUsuario.CodUsuario;
      oLog.ObsLog = "TOP 50 CLIENTES MEJOR ÍNDICE DE CREDIBILIDAD";
      oLog.CodEvtLog = "107";
      oLog.AppLog = "TOP 50 CLIENTES MEJOR ÍNDICE DE CREDIBILIDAD";
      oLog.putLog();
    }
    ...
    protected void get20CliMejorIndiceCredibilidad()
    {
      ...
      if (oConn.Open())
      {
        ...
        DataTable dt = oTopCliMayMntPastDue.getTopMejorCredibilidad();
        if (dt != null)
        {
          if (dt.Rows.Count > 0)
          {
            ...
            foreach (DataRow oRow in dt.Rows)
            {
              ...
              sHtml = sHtml + "<div class='blq-dt-pastdue'><span class='dt-pastdue'> " + Math.Truncate(double.Parse(oRow["cumplidas"].ToString())).ToString("N0") + "</span></div>";
              sHtml = sHtml + "</div>";
              sHtml = sHtml + "</a>";
              sHtml = sHtml + "</div>";
