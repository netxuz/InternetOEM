...
{
  public partial class clientes_menor_credibilidad : System.Web.UI.Page
  {
    ...
    protected void get20CliMenorCredibilidad()
    {
      ...
      if (oConn.Open())
      {
        ...
        DataTable dt = oTopCliMayMntPastDue.getTopMenorCredibilidad();
        if (dt != null)
        {
          if (dt.Rows.Count > 0)
          {
            ...
            foreach (DataRow oRow in dt.Rows)
            {
              ...
              sHtml = sHtml + "<div class='blq-dt-pastdue'><span class='dt-pastdue'> " + Math.Truncate(double.Parse(oRow["no_cumplidas"].ToString())).ToString("N0") + "</span></div>";
              sHtml = sHtml + "</div>";
              sHtml = sHtml + "</a>";
              sHtml = sHtml + "</div>";
