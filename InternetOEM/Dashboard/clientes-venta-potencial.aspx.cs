...
{
  public partial class clientes_venta_potencial : System.Web.UI.Page
  {
    ...
    protected void get50CliVentaPotencial()
    {
      ...
      if (oConn.Open())
      {
        ...
        DataTable dt = oTopCliMayMntPastDue.getTopVentaPotencial();
        if (dt != null)
        {
          if (dt.Rows.Count > 0)
          {
            ...
            foreach (DataRow oRow in dt.Rows)
            {
              ...
              sHtml = sHtml + "<div class='blq-dt-pastdue'><span class='dt-pastdue'>$ " + Math.Truncate(double.Parse(oRow["linea_credito_actual"].ToString())).ToString("N0") + "</span></div>";
              sHtml = sHtml + "</div>";
              sHtml = sHtml + "</a>";
              sHtml = sHtml + "</div>";
