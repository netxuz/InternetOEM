...
{
  public partial class clientes_mayor_monto_pastdue_critico : System.Web.UI.Page
  {
    ...
    protected void get20CliMayMntPastDueCritico()
    {
      ...
      if (oConn.Open())
      {
        ...
        DataTable dt = oTopCliMayMntPastDue.getTop20cli_may_pastdue_critico();
        if (dt != null)
        {
          if (dt.Rows.Count > 0)
          {
            ...
            foreach (DataRow oRow in dt.Rows)
            {
              ...
              sHtml = sHtml + "<div class='blq-dt-pastdue'><span class='dt-pastdue'>$ " + Math.Truncate(double.Parse(oRow["past_due_critico"].ToString())).ToString("N0") + "</span></div>";
              sHtml = sHtml + "</div>";
              sHtml = sHtml + "</a>";
              sHtml = sHtml + "</div>";
