  ...
  public class cClientesRetenidos
  {
    ...
    public DataTable Get()
    {
      ...
      if (oConn.bIsOpen)
      {
        ...
        cSQL.Append(" (select sum(saldo) from vista_antiguedad_deuda_new where nkey_cliente =").Append(lngCodNkey);
        cSQL.Append(" and nkey_deudor =  LineaCredito.nKey_Deudor) AS 'DeudaTotal', (select sum(saldo) from vista_antiguedad_deuda_new where nkey_cliente =").Append(lngCodNkey);
        ...
        cSQL.Append(" (select sum(saldo*dias_atraso) / sum(saldo) from vista_saldo_factura v1 where nkey_cliente =").Append(lngCodNkey);
        cSQL.Append(" and v1.nkey_deudor = LineaCredito.nKey_Deudor and v1.saldo > 0 and v1.dias_atraso > 0 ) as 'DiasPonderado' ");
        cSQL.Append(" FROM LineaCredito JOIN CodigoDeudor ON(CodigoDeudor.nKey_Cliente = ").Append(lngCodNkey).Append(" AND CodigoDeudor.nKey_Deudor = LineaCredito.nKey_Deudor) ");
        cSQL.Append(" JOIN Deudor ON (Deudor.nKey_Deudor = LineaCredito.nKey_Deudor) LEFT JOIN CategoriaRiesgo ON(LineaCredito.nKey_Cliente = ").Append(lngCodNkey).Append(" AND LineaCredito.nKey_Deudor = CategoriaRiesgo.nKey_Deudor) ");
        ...
        cSQL.Append(" JOIN CLIENTE ON (cliente.nkey_cliente = ").Append(lngCodNkey).Append(") ");
        cSQL.Append(" LEFT JOIN gestiondeudor ON (gestiondeudor.nkey_cliente = ").Append(lngCodNkey);
        ...
        cSQL.Append(" gestiondeudor GD where GD.nkey_cliente = ").Append(lngCodNkey).Append(" and GD.nkey_deudor = gestiondeudor.nkey_deudor )) ");
        ...
        cSQL.Append(" WHERE LineaCredito.sEstatusCredito LIKE '%Retenido%' AND LineaCredito.nKey_Cliente = ").Append(lngCodNkey);
