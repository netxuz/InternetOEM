...
{
  public partial class app_dist_ret_doc_find : System.Web.UI.Page
  {
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
      ...
      if (!IsPostBack)
      {
        ...
        oLog.ObsLog = "REPORTE DE DISTRIBUCIÓN Y RETIRO DE DOCUMENTOS";
        oLog.CodEvtLog = "1";
    ...
    protected void idBuscar_Click(object sender, EventArgs e)
    {
      ...
      rdGridDistRetDoc.Rebind();
    }

    protected void rdGridDistRetDoc_NeedDataSource(object source, GridNeedDataSourceEventArgs e)
    {
      rdGridDistRetDoc.DataSource = getDatatble();
    }

    protected void rdGridDistRetDoc_ItemCommand(object source, GridCommandEventArgs e)
    {
    ...
    protected void rdGridDistRetDoc_ItemDataBound(object sender, GridItemEventArgs e)
    {
    ...
    public DataTable getDatatble()
    {
      ...
      if (oConn.Open())
      {
        cDistribucionRetiroDocumentos oDistribucionRetiroDocumentos = new cDistribucionRetiroDocumentos(ref oConn);
        oDistribucionRetiroDocumentos.CodDeudor = hddCodDeudor.Value;
        oDistribucionRetiroDocumentos.CodNkey = ((!string.IsNullOrEmpty(cmbCliente.SelectedValue) ? cmbCliente.SelectedValue : hdd_arrNkeyCliente.Value));
        oDistribucionRetiroDocumentos.NcodHolding = cmbHolding.SelectedValue;
        oDistribucionRetiroDocumentos.NkeyUsuario = oIsUsuario.NKeyUsuario;
        oDistribucionRetiroDocumentos.TipoUsuario = oIsUsuario.TipoUsuario;
        oDistribucionRetiroDocumentos.DtFchIni = DateTime.Parse(RadDatePicker1.SelectedDate.ToString()).ToString("yyyyMMdd");
        oDistribucionRetiroDocumentos.DtFchFin = DateTime.Parse(RadDatePicker2.SelectedDate.ToString()).ToString("yyyyMMdd");
        dt = oDistribucionRetiroDocumentos.Get();
    ...
    protected void Page_PreRender(object o, EventArgs e)
    {
      ...
      rdGridDistRetDoc.MasterTableView.GetColumn("ncodholding").Display = true;
      rdGridDistRetDoc.MasterTableView.GetColumn("código").Display = true;
      rdGridDistRetDoc.MasterTableView.GetColumn("Razón_Social").Display = true;

      if (!string.IsNullOrEmpty(hddCodDeudor.Value))
      {
        rdGridDistRetDoc.MasterTableView.GetColumn("ncodholding").Display = false;
        rdGridDistRetDoc.MasterTableView.GetColumn("código").Display = false;
        rdGridDistRetDoc.MasterTableView.GetColumn("Razón_Social").Display = false;
      }
